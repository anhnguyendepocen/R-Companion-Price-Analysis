...
In the Console type the following to get aquainted with performing
arithmetic in R.
...
The function, `data.table()` takes the rectagular object defined by the
`cbind()` function and makes it into a data frame. The arguement,
...
![](Chapter1_files/figure-markdown_strict/unnamed-chunk-19-1.png)
~~~~~~~~
...
chapter, you are well equiped to explore further if you are interested.
...
1.  Create a fake data frame similar to our one named `DATA`, exept this
    fake data should contain columns named `Age` (in years), `City`
    (note that these elements should be character strings), `Income` (in
    U.S. dollars), and `HSDiploma` (make this a logical, `TRUE` for yes
    and `FALSE` for no).

Again, I want you to make up the data that goes in the data frame, we
are checking here if you can build a data frame of similar structure to
`DATA` from the example above. Save this code in your Chapter1.R script
file.
